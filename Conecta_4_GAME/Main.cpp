...
{
	string idUsu, clave;
	if(gestor.listaUsuarios.contador<MAXUSU){
		datosUsuario(gestor, idUsu, clave);
...
{
	int partida;
	if (gestor.listaUsuarios.LisUsu[gestor.posUsuario].lista.contador == 0) cout << "No existe ninguna partida" << endl;
	else{
		do{
			cout << "Seleccion la partida que desea ver: ";
			cin >> partida;
			if (partida > gestor.listaUsuarios.LisUsu[gestor.posUsuario].lista.contador || partida <= 0) cout << "Lo siento esa partida no existe" << endl;
		} while (partida <=0 || partida > gestor.listaUsuarios.LisUsu[gestor.posUsuario].lista.contador);
...
void opcionJugar(tGestor &gestor,ostringstream &aviso){
	...
	if(asterisco(gestor.listaPartidas.lisPar[gestor.accPar],gestor.listaUsuarios.LisUsu[gestor.posUsuario].id)){
